...
def RPeakDetection(ecg):
    '''
    Detect R peaks in input signal
    ...
    Returns:
      out: list
          A list of locations of detected R peaks in ecg.
          out[0] --> The number of detected R peaks.
          out[1:] --> Locations of R peaks in input signal.
    '''
    ...
    QRSLocationArray[-1] = len(QRSLocationArray)  ##最後一筆設定為array長度，避免後減前產生錯誤判斷
